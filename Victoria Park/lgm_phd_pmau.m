function [obj,est] = lgm_phd_pmau(obj,y,u,t,params,ukf)
    ...
    [obj_i, ukf] = lpmau_predict(obj(1),u,t,params,ukf);
    ...
    if iParent == 0 || iParent ~= iPrevParent
        if params.J == 0 || size(obj_i.xl,2) == 0 || size(y,2) == 0
            ...
            % compute cost matrix
            [P_D, ~] = cost_matrix(obj_i, y, params);
            
            % compute parameters of GM-ID approximation
            [wp,mp,Pp,~] = lpmauiplid(obj_i,y,log(P_D),params,ukf,t(end));
        end
        iPrevParent = iParent;
    end
    ...
    obj_i = lUpdate(obj_i,y,params,t(end));

    % prune and merge PHD components, and store the particle in the 
    % PHD-SLAM density object
    obj(1) = lHypothesisReductionAlgorithm(obj_i,params,t(end));
